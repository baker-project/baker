...
class Detector:
    ...
    def __init__(self, name, topic, frequency=0.2, dirt_positions_candidates=[]):
        ...
        print(name)
        print(self.dirt_positions_candidates_)
        print(self.dirt_positions_candidates_)
        print(self.dirt_positions_candidates_)
        print(self.dirt_positions_candidates_)
    ...
    def talker(self):
        ...
        while not rospy.is_shutdown():
            ...
            detections = randomDetections() if self.use_random_detections_ else self.fixedDetections()
    ...
    try:
        ...
        if '--dirt' in args:
            ...
            dirt_detector = Detector('dirt_detection_server_preprocessing', '/dirt_detection_server_preprocessing/dirt_detector_topic', 1000,
                dirt_positions_candidates=[(-6.24, -1.33, 0)])
        if '--trash' in args:
