...
ImageBlend::ImageBlend(std::string& clean_ground_path, std::string& artificial_dirt_path,
                       std::string& artificial_dirt_mask_path, std::string& segmented_pens_path,
		       std::string& segmented_pens_mask_path, std::string& blended_img_folder,
		       std::string& blended_mask_folder, int max_num_dirt, int min_num_dirt, 
		       int max_num_pens, int min_num_pens, std::string& filename_bbox, bool flip_clean_ground,
...
{ 
  ...
  num_pens_images_ = std::distance(boost::filesystem::directory_iterator(artificial_pens_images),
  					      boost::filesystem::directory_iterator{}); 
  ...
  for (int m = 0; m < clean_ground_filenames_.size() ; m++)
  {
    ...
    for (vector<std::string>::iterator it1 = strs1.begin(); it1 != strs1.end() - 1; it1++)
    {
...
void ImageBlend::blendImage(int dirt_num, std::ofstream& myfile, std::string& ground_file_name)
{
  ...
  for (int n = 0; n < dirt_num; n++)
  {
    ...
    cv::Mat element = cv::getStructuringElement( cv::MORPH_RECT, Size(3 , 3), Point(1, 1) );
    ...
    myfile << ground_file_name + "s" + DatasetCreate::to_string(serie_num_) << " " << anchor_col - 2 << " " << anchor_row - 2 << " "
           << anchor_col + dirt_cols + 2 << " " << anchor_row + dirt_rows + 2 << " " << "Dirt \n";
...
void ImageBlend::blendImage(int dirt_num, std::ofstream& myfile, std::string& ground_file_name, bool if_for_classification)
{
  int img_cols = clean_ground_pattern_.cols;
  int img_rows = clean_ground_pattern_.rows;
  ...
  for (int n = 0; n < dirt_num; n++)
  {
    ...
    // write the arguments of the bounding box in file
    myfile << ground_file_name + "s" + DatasetCreate::to_string(serie_num_) << " " << anchor_col << " " << anchor_row << " "
           << anchor_col + dirt_cols << " " << anchor_row + dirt_rows << " " << class_name << '\n';
...
void ImageBlend::shrank_bounding_box()
{
...
void ImageBlend::get_patch_classname(std::string& patch_name, std::string& class_name)
{
  std::vector<std::string> strs; 
...
void ImageBlend::edge_smoothing(int half_kernel_size)
{
  ...
  */
  ...
  for (int contour_iter = 0; contour_iter < num_contours; contour_iter++)
  {
    ...
    for (int point_iter = 0; point_iter < num_points; point_iter++)
    {
      ...
      int point_x = anchor_point.x; int point_y = anchor_point.y;
      
      if ((point_x - half_kernel_size) < 0 || (point_x + half_kernel_size) >= img_cols_||
	  (point_y - half_kernel_size) < 0 || (point_y + half_kernel_size) >= img_rows_)      // if the ROI is out of the image range, continue
	continue;
      
      cv::Mat image_roi;
      cv::Rect region_of_interest(point_x - half_kernel_size, point_y - half_kernel_size, 
				  2 * half_kernel_size + 1, 2 * half_kernel_size + 1);
...
void ImageBlend::shadow_and_illuminance(bool shadow_or_illuminance)
{
  ...
  if (polygon_or_ellipse)                        // assume the polygon with 4 points
  {
    ...
    int start_point_x = rand() % img_cols_;  int start_point_y = rand() % img_rows_;
    float x1 = rand() % abs(start_point_x - 80) + 40;     float y1 = rand() % abs(start_point_y - 80) + 40;
    float x2 = rand() % abs(start_point_x - 80) + 40;     float y2 = rand() % abs(abs(start_point_y - img_rows_) - 80) + 40;
    float x3 = rand() % abs(abs(start_point_x - img_cols_)- 80) + 40;     float y3 = rand() % abs(abs(start_point_y - img_rows_) - 80) + 40;
    float x4 = rand() % abs(abs(start_point_x - img_cols_) - 80) + 40;     float y4 = rand() % abs(start_point_y - 80) + 40;      
  ...
  {
    int start_x = rand() % img_cols_;   int start_y = rand() % img_rows_;        // the minimum length of the axis is 40, maximun is 80
    int axes_x = rand() % 80 + 60;   int axes_y = rand() % 80 + 60;              // TODO check if out of image range cause any problems
    
    int angle = rand() % 360;
    int start_angle = rand() % 90;   int end_angle = rand() % 180 + 180;
...
void ImageBlend::shadow_and_illuminance_new(bool shadow_or_illuminance)
{
