...
public:
	BoundingBoxLabeler(const std::string& winName, cv::Mat& imageWithRects, cv::Mat& originalImage, cv::Scalar clr) :
	...
	static void writeTxt(const std::vector<BoundingBoxLabeler>& all, const std::string& path, const std::string& filename);

	static void writeXml(std::vector<BoundingBoxLabeler> all, std::string path, std::string actTime);

	static void writeXml3d(const std::vector<BoundingBoxLabeler>& all, std::string path, std::string actTime);

	static void readXml3d(std::vector<BoundingBoxLabeler>& groundTruthData, std::string filename);
